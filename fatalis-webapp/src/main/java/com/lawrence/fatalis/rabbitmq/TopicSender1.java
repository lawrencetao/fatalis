...
public class TopicSender1 implements RabbitTemplate.ConfirmCallback {
    ...
    public TopicSender1(RabbitTemplate rabbitTemp) {
    ...
    public void sendMessage1(TestObj message) {
        ...
        rabbitTemplate.convertAndSend(AmqpConfig.TOPIC_EXCHANGE, "topic.queue1", message, correlationData);
    }

    public void sendMessage2(TestObj message) {
        CorrelationData correlationData = new CorrelationData(StringUtil.getUUIDStr());
        rabbitTemplate.convertAndSend(AmqpConfig.TOPIC_EXCHANGE, "topic.any", message, correlationData);
    }

    public void sendMessage3(TestObj message) {
        CorrelationData correlationData = new CorrelationData(StringUtil.getUUIDStr());
        rabbitTemplate.convertAndSend(AmqpConfig.TOPIC_EXCHANGE, "any.queue2", message, correlationData);
    }
